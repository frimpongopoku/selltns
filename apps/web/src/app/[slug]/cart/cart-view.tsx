...
export function CartView({ tenantId }: { tenantId: string }) {
  ...
  // already sitting in someone's cart, so every time this page is opened
  // it's cross-checked against the live product list rather than trusting
  ...
  useEffect(() => {
    ...
    pruneStaleCartLines(tenantId, lines, removeItem)
      ...
      .catch(() => {
        // A failed check shouldn't block browsing the cart — checkout
        // re-validates again at submit time regardless.
      });
